...
#requests.get("https://github.com/chatopera/Synonyms/releases/download/3.15.0/words.vector.gz", verify=False)
...
#src_db=MySQLdb.connect(host=db_host,user=db_user,passwd=db_passwd,db="qiwsirtest",port=db_port,charset=db_charset)

curdir = os.path.dirname(os.path.abspath(__file__))#os.path.abspath(__file__)脚本的绝对路径
                                                   #os.path.dirname(path)path的父路径
sys.path.append(curdir)#模块和自己写的脚本不在同一个目录下，故在sys的path列表中加入 这里很重要，关系到能不能找到词向量文件words.vector.gz

if sys.version_info[0] < 3:#判断python是否为3以下 复现环境使用python2.7
    reload(sys)
    sys.setdefaultencoding("utf-8")#将编码设置为utf8
...
def is_sort(interval,s2,mywords1,mywords2):#判断关键词1和关键词2之间的距离约束是否满足
    list1 = []
    list2 = []
...
def detect(sen2):
	# sen2 = ''  # input: 评论
	s2 = [x for x in jieba.cut(sen2)]#分词
	n = 0
	while 1:
		if n >= len(s2):
			break
