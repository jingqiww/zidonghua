...
sys.path.append(curdir)#模块和自己写的脚本不在同一个目录下，故在sys的path列表中加入
...
stopwords = []
for word in open('en_stopwords.txt', 'r'):#只读方式打开
...
def is_in_list(s2,mywords):#判断某关键词是否在关键词集中，由于英文多了一步去除空格且英文多了单复数、时态的变化，故比中文新增这一方法判断
    i = 0
    for words in s2:
        for myword in mywords:
...
def detect(sen2):
	#sen2=输入的评论
	s2 = sen2.split(' ')#按照空格将评论分割成单词
	n = 0
	while 1:
		if n >= len(s2):
			break
		else:
			if s2[n] in stopwords:#去除停词
				s2.remove(s2[n])
				n = 0
			else:
				n = n + 1
	...
	if len(s2) > 0:
		...
		virusrule1 = is_in_list(s2, ["virus"])
        	virusrule2 = is_in_list(s2, ["trojan"])
